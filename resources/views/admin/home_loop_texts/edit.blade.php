@extends('layouts.app')
@section('title', 'Editar Texto Loop')
@section('content')
    <div class="bg-white shadow sm:rounded-lg p-6">
        <h3 class="text-lg font-medium text-gray-900">Editar Texto Loop</h3>
        <form action="{{ route('admin.home_loop_texts.update', $homeLoopText->id) }}" method="POST" class="mt-5 space-y-6">
            @csrf @method('PUT')
            <div>
                ...
                <textarea name="content" rows="4"
                    ...
                    required>{{ $homeLoopText->content }}</textarea>
            </div>
            <div class="flex items-center">
                <input id="is_active" name="is_active" type="checkbox" value="1"
                    {{ $homeLoopText->is_active ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
